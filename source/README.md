...
- **Key art (2026-09-10, Bennet):** `web/key-fertility.jpg` = the ultrasound reference (header-cropped) with FERTILITY set in the site serif, yellow-orange→red gradient, across the dark upper-middle space. Source page `source/key-fertility-art.html` (render at 1760×990, DPR 1, then JPEG q88). The previous Leonardo-drawing art is kept as `web/key-fertility-leonardo.jpg`. The slate card uses plain `.sfilm-art` (no overlaid title) because the art carries the title.
