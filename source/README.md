...
- **Key art (2026-09-10, Bennet; v3):** `web/key-fertility.jpg` = the ultrasound reference (header-cropped) with FERTILITY set in the site serif across the dark band between the probe arc and the head (`top:26%`), gradient `#ffe374 → #ffbd3a → #f2701f → #d8351a` (yellow to red), plate `contrast(1.3) brightness(.95) saturate(.9)`. Source page `source/key-fertility-art.html` (render at 1760×990, DPR 1, then JPEG q88). Ships as `fertility/img/key-fertility-v3.jpg` — bump the suffix on every change so browser/edge caches (max-age 14400) miss. /design-review on the image: PASS (weakest gradient stop 3.09:1 vs its ground; one focal point). The previous Leonardo-drawing art is kept as `web/key-fertility-leonardo.jpg`. The slate card uses plain `.sfilm-art` (no overlaid title) because the art carries the title.
