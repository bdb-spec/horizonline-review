...
- **Deck:** `source/fertility-pdf/FERTILITY-DIRECTORS-VISION.pdf` → `fertility/FERTILITY-DIRECTORS-VISION.pdf`. The page text is the Aug-18 "A Bloodline" master's copy (Bennet confirmed 2026-09-10), ported verbatim. **The hosted PDF is a redacted copy** (PyMuPDF pixel redaction of the ultrasound header band on p.4, which carried a patient name/ID/timestamp; md5 `3693db79…`). Bennet's original bytes (md5 `2a041427…`) are kept undeployed as `source/fertility-pdf/FERTILITY-DIRECTORS-VISION-2a041427-original.pdf`; `build.py` copies only the redacted file. The web `ultrasound.jpg` is cropped 9% from the top by `prep-fertility-img.py` for the same reason. Never publish the header.
